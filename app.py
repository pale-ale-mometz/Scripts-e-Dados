...
# --- 2. MAIN DASHBOARD & SMART TIME LOGIC ---
st.title("📊 Vendas Dashboard")

# Calculate São Paulo time (UTC - 3 hours)
now_utc = datetime.datetime.utcnow()
now_sp = now_utc - datetime.timedelta(hours=3)

# If it's past 11:30 AM in SP, data is up to D-1. Otherwise, D-2.
...
st.caption(f"🔄 **Última atualização do Banco de Dados:** {hora_atualizacao} (Dados consolidados até {reference_date.strftime('%d/%m/%Y')})")
...
def load_data():
    ...
    query = f"""
    SELECT data_venda, uf, tipo_venda, Vendas
...
def load_invest_data():
    start_of_last_year = datetime.date.today().replace(year=datetime.date.today().year - 1, month=1, day=1)
    query = f"""
    SELECT data_investimento, canal, plataforma, branding, leads, venda, vol_leads, vol_vendas
...
with tab1:
    ...
    opcoes_canais_dinamico = sorted([str(c).title() for c in df['tipo_venda'].dropna().unique()])
    canais_selecionados = st.multiselect("Filtre os canais desejados:", options=opcoes_canais_dinamico, default=opcoes_canais_dinamico)
    ...
    df_c, df_p, df_l = df_current.copy(), df_prev_full.copy(), df_last_full.copy()
    ...
    df_c_grp['Período'], df_p_grp['Período'], df_l_grp['Período'] = 'Atual', 'Anterior', 'Ano Passado'
    dfs_to_concat = [df_c_grp, df_l_grp] if view_option == "Ano Atual" else [df_c_grp, df_p_grp, df_l_grp]
    ...
    # --- RECORDES HISTÓRICOS (TOP 5) ---
    st.subheader("🏆 Recordes Históricos (Top 5)")
    st.markdown("Ranking dos melhores períodos de vendas considerando toda a base de dados (desde Janeiro do ano passado).")
    
    gran_rec = st.radio("Selecione o agrupamento do ranking:", ["Dias", "Semanas", "Meses"], horizontal=True)
    
    df_rec_tot = df.copy()
    df_rec_can = df[df['tipo_venda'].astype(str).str.lower().str.strip().isin(canais_alvo)].copy()
    
    def get_top5(df_in, gran):
        if df_in.empty:
            return pd.DataFrame(columns=["Período", "Vendas"])
        if gran == "Dias":
            grp = df_in.groupby('data_venda')['Vendas'].sum().reset_index()
            grp['Período'] = grp['data_venda'].dt.strftime('%d/%m/%Y')
        elif gran == "Semanas":
            grp = df_in.groupby(pd.Grouper(key='data_venda', freq='W-MON'))['Vendas'].sum().reset_index()
            grp['Período'] = "Semana de " + grp['data_venda'].dt.strftime('%d/%m/%Y')
        else:
            grp = df_in.groupby(pd.Grouper(key='data_venda', freq='ME'))['Vendas'].sum().reset_index()
            grp['Período'] = grp['data_venda'].dt.strftime('%m/%Y')
            
        top = grp.sort_values(by='Vendas', ascending=False).head(5)
        top['Vendas'] = top['Vendas'].apply(format_br)
        return top[['Período', 'Vendas']].reset_index(drop=True)

    col_t1, col_t2 = st.columns(2)
    with col_t1:
        st.markdown("**Geral (Todos os Canais)**")
        st.dataframe(get_top5(df_rec_tot, gran_rec), use_container_width=True)
    with col_t2:
        st.markdown("**Canais Selecionados Acima**")
        st.dataframe(get_top5(df_rec_can, gran_rec), use_container_width=True)
...
# =====================================================================
# TAB 3: COMPOSIÇÃO POR TIPO DE VENDA
# =====================================================================
with tab3:
...
with tab4:
    ...
    if not df_graficos_inv.empty:
        ...
        fig_line = px.line(df_line_grp, x='data_investimento', y='Y', markers=True, color_discrete_sequence=['#4C78A8'])
